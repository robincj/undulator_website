<?php
// Build img tag blocks for all images to include in the JSSOR slider display
...
$count = 0;
$imgHtml = '';
while (count($imgFileList) && $count < $displayNum && $count <= count($imgFileList)) {
    $rand = rand(1, count($imgFileList) - 1);
    $file = $imgFileList[$rand];
    $imgHtml .= "<div><img u='image' src2='$file' /></div>\n";
    $count ++;
}
